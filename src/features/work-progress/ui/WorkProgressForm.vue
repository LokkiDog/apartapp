    ...
    <section v-if="kind === 'cleaning'" class="progress-section">
      ...
      <div v-if="inventoryReports.length" class="progress-stock-list space-y-3">
        <div v-for="(item, index) in inventoryReports" :id="`progress-stock-${item.consumable.id}`" :key="item.consumable.id" class="progress-stock-row" :class="{ 'progress-stock-row--focused': focusConsumableId === item.consumable.id }" :tabindex="focusConsumableId === item.consumable.id ? -1 : undefined">
          <div class="progress-stock-main">
            <div class="progress-stock-identification"><p class="progress-stock-name">{{ item.consumable.name }}</p><p class="progress-stock-current">{{ t('progress.current') }}: <span>{{ item.quantity }}</span><span v-if="item.autoWriteOffQuantity !== null"> · {{ t('inventory.auto') }}: {{ item.autoWriteOffQuantity }}</span></p></div>
            <UFormField :name="`inventoryReports.${index}.usedQuantity`" :error="false" class="progress-stock-field"><template #label><span class="progress-stock-label--desktop">{{ t('progress.used') }}</span><span class="progress-stock-label--mobile">{{ t('progress.usedShort') }}</span></template><UInput v-model.number="item.usedQuantity" type="number" min="0" step=".001" :disabled="!editable && !inventoryEditable" class="progress-stock-input" @update:model-value="syncExpectedRemaining(item)" /></UFormField>
            <UFormField :name="`inventoryReports.${index}.remainingQuantity`" :error="false" class="progress-stock-field"><template #label><span class="progress-stock-label--desktop">{{ t('progress.remaining') }}</span><span class="progress-stock-label--mobile">{{ t('progress.remainingShort') }}</span></template><UInput v-model.number="item.remainingQuantity" type="number" min="0" step=".001" :disabled="!editable && !inventoryEditable" class="progress-stock-input" @update:model-value="item.remainingTouched = true" /></UFormField>
          </div>
          <div v-if="focusConsumableId === item.consumable.id && item.report" class="progress-stock-source mt-3">
            ...
            <dl class="mt-3 grid grid-cols-1 gap-2 text-sm sm:grid-cols-3"><div><dt>{{ t('inventoryDiscrepancy.beforeCleaning') }}</dt><dd>{{ item.report.startingQuantity }}</dd></div><div><dt>{{ t('inventoryDiscrepancy.expected') }}</dt><dd>{{ item.report.expectedRemainingQuantity }}</dd></div><div><dt>{{ t('inventoryDiscrepancy.reported') }}</dt><dd>{{ item.report.remainingQuantity }}</dd></div></dl>
          </div>
          <p v-if="focusConsumableId === item.consumable.id" class="mt-2 text-xs text-[var(--color-muted)]">{{ t('inventoryDiscrepancy.expectedAfterCorrection') }}: {{ liveExpectedRemaining(item) }}</p>
          ...
          <p v-else-if="liveDiscrepancy(item)" class="mt-2 text-xs text-amber-700">{{ t('progress.discrepancy') }}: {{ liveDiscrepancy(item) > 0 ? '+' : '' }}{{ liveDiscrepancy(item) }}</p>

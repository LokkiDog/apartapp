...
type ApartmentChoice = Pick<Apartment, "id" | "name"> & {
  hotel: Pick<Apartment["hotel"], "name">;
...
const items = computed(() => {
  ...
  const sortedApartments = [...props.apartments].sort(
    (left, right) =>
  ...
  const apartmentGroups = [...apartmentsByHotel.entries()].map(
    ([hotelName, apartments]) => [
      {
        type: "label" as const,
        label: hotelName,
      ...
      ...apartments.map((apartment) => ({
        ...
        value: apartment.id,
        search: apartmentSearchText({
          name: apartment.name,
          hotelName: apartment.hotel.name,
        }),
        description:
          apartment.status === "archived" ? t("scope.archived") : undefined,
      })),
    ],
  );
  const prependGroup = props.prependItems.map((item) => ({
    ...item,
    search: item.label,
  }));

  return prependGroup.length
    ? [prependGroup, ...apartmentGroups]
    : apartmentGroups;
});
...
function selectedLabel(value: unknown) {
  if (Array.isArray(value)) {
    ...
    return value.length === 1
      ? t("scope.selectedOne")
      : t("scope.selectedMany", { count: value.length });
  }
...
<template>
  <USelectMenu
    ...
    :search-input="{
      placeholder: t('common.searchApartment'),
      variant: 'none',
      ui: {
        root: 'm-2 w-auto self-stretch',
        base: 'h-10 min-h-0 border-0 px-3 py-0 text-sm ring-0 focus:ring-0 focus-visible:ring-0',
      },
    }"
    :content="{
      align: 'start',
    ...
    :ui="{
      base: 'w-full min-h-11 rounded-[10px]',
      value: 'w-full min-w-0 text-start',
      placeholder: 'w-full min-w-0 text-start',
      content: 'apartment-select-menu rounded-[12px]',
      viewport: 'min-h-0 overflow-y-auto overscroll-contain',
      label:
        'sticky top-0 z-[1] bg-default px-2.5 py-1.5 text-xs font-semibold text-[var(--color-muted)]',
      item: 'min-h-10 items-center px-2.5 py-1.5',
    }"
    class="w-full"
  >
    <template #default="{ modelValue: selectedValue }">
      <span
        class="pointer-events-none w-full min-w-0 truncate text-start"
        :class="{ 'text-[var(--color-muted)]': !selectedLabel(selectedValue) }"
        >{{ selectedLabel(selectedValue) || placeholder || "\u00a0" }}</span
      >
    </template>
    <template #empty
      ><span class="block px-3 py-2 text-sm text-[var(--color-muted)]">{{
        t("common.noApartmentsFound")
      }}</span></template
    >
  </USelectMenu>
</template>
...
.apartment-select-menu {
  max-height: min(
    18rem,
    calc(100dvh - 1rem),
    var(--reka-combobox-content-available-height, 18rem)
  );
}

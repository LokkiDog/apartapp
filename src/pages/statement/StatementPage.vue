...
const addItems = computed<DropdownMenuItem[][]>(() => [managerExpenseCategories.map(category => ({ label: categoryLabels[category], icon: category === 'cleaning' ? 'i-lucide-sparkles' : category === 'inventory' ? 'i-lucide-package' : 'i-lucide-wrench', onSelect: () => addLine(category) }))])
...
async function loadReport() {
  ...
  } catch (cause: any) { error.value = cause?.data?.statusMessage ?? 'Не удалось загрузить отчёт'; report.value = null; draftLines.value = []; draftCategoryVisibility.value = createManagerExpenseCategoryVisibility() }
  finally { loading.value = false }
}
...
async function downloadPdf() {
  ...
  } catch (cause: any) { error.value = cause?.message ?? 'Не удалось подготовить PDF' }
...
<template>
  <section class="page-wrap space-y-6">
    <PageHeader :title="isAdministrator ? 'Расходы управляющих' : 'Мои расходы'" :description="isAdministrator ? 'Подготовьте и откройте управляющему месячный отчёт по апартаменту.' : 'Опубликованные отчёты по закреплённым за вами апартаментам.'">
      <template #actions><div class="flex flex-wrap items-center gap-2"><UInput v-model="month" type="month" class="w-44" /><UButton v-if="!isAdministrator && report" color="neutral" variant="outline" icon="i-lucide-download" class="min-h-11" :loading="pdfPending" @click="downloadPdf">Скачать PDF</UButton></div></template>
    </PageHeader>
      ...
      <div v-else-if="reports.length" class="manager-expense-layout grid gap-3">
        <aside class="surface divide-y divide-[var(--color-line)] overflow-hidden">
          <button v-for="item in reports" :key="item.apartmentId" type="button" class="flex min-h-18 w-full items-center justify-between gap-3 px-4 py-3 text-left transition-colors hover:bg-[var(--color-surface-muted)]" :class="{ 'bg-[var(--color-primary-soft)]': selectedApartmentId === item.apartmentId }" @click="selectedApartmentId = item.apartmentId">
            <span class="min-w-0"><strong class="block truncate">{{ item.apartmentName }}</strong><span class="mt-0.5 block truncate text-sm text-[var(--color-muted)]">{{ item.managerNames.join(', ') || 'Управляющие не назначены' }}</span></span>
        ...
        <div v-if="report" class="surface overflow-hidden">
          <div class="manager-expense-toolbar border-b border-[var(--color-line)] p-4 sm:p-5">
            <div class="manager-expense-toolbar__heading flex min-w-0 flex-wrap items-center gap-x-4 gap-y-1">
              <div class="min-w-0"><h2 class="truncate font-semibold">{{ report.apartment.name }}</h2><p class="mt-1 truncate text-sm text-[var(--color-muted)]">{{ report.apartment.managerNames.join(', ') || 'Управляющие пока не назначены' }}</p></div>
            ...
            <div class="manager-expense-actions">
              ...
              <UButton color="neutral" variant="outline" class="min-h-11" :loading="pending" @click="resetOpen = true">Пересобрать</UButton>
              <UButton class="min-h-11" :loading="pending" @click="save">Сохранить</UButton>
              <UButton color="neutral" variant="outline" icon="i-lucide-download" class="min-h-11" :loading="pdfPending" :disabled="hasUnsavedChanges" :title="hasUnsavedChanges ? 'Сначала сохраните изменения' : undefined" @click="downloadPdf">Скачать PDF</UButton>
          ...
          <div class="px-4 pt-3 sm:px-5"><p class="text-sm font-medium text-[var(--color-muted)]">Превью управляющего. Нажмите строку, чтобы изменить.</p></div>

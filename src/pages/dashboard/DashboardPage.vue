...
const [{ data: stays, status: staysStatus }, { data: cleanings }, { data: tasks }] = await Promise.all([
  ...
  useAsyncData('dashboard-cleanings', () => $fetch<Cleaning[]>('/api/cleanings'), { server: false, default: () => [], watch: [currentUser] }),
  useAsyncData('dashboard-tasks', () => $fetch<Task[]>('/api/tasks'), { server: false, default: () => [], watch: [currentUser] })
])
...
const monthLabel = computed(() => dashboardMonthLabel(dashboardMonth.value))
...
const undatedWork = computed(() => sortDashboardRecords([
  ...(cleanings.value ?? []).filter(item => (activeCleaning(item.status) || item.hasProblem) && undatedDashboardRecord(item)),
...
<template>
  <section class="page-wrap space-y-6">
    <PageHeader class="dashboard-page-header" :title="isCleanerOnly ? `Мои работы · ${monthLabel}` : `${monthLabel} в апартаментах`" :description="isCleanerOnly ? 'Назначенные работы и задачи за выбранный месяц.' : 'Заезды, уборки и задачи, которым нужно внимание.'"><template #actions><div class="dashboard-page-header__actions"><div class="dashboard-month-control" aria-label="Выбор месяца"><UButton color="neutral" variant="soft" icon="i-lucide-chevron-left" aria-label="Предыдущий месяц" @click="shiftMonth(-1)" /><span class="dashboard-month-control__label">{{ monthLabel }}</span><UButton color="neutral" variant="soft" icon="i-lucide-chevron-right" aria-label="Следующий месяц" @click="shiftMonth(1)" /><UButton v-if="dashboardMonth !== currentMonth" color="neutral" variant="ghost" @click="resetMonth">Текущий месяц</UButton></div><UButton v-if="!isCleanerOnly" to="/calendar" icon="i-lucide-plus">Новый заезд</UButton></div></template></PageHeader>

  ...
  <section class="page-wrap space-y-6">
    ...
    <div v-else-if="visibleUsers.length" class="surface divide-y divide-[var(--color-line)] px-5 sm:px-6">
      <div v-for="member in visibleUsers" :key="member.id" class="flex min-h-20 items-center gap-4 py-4">
        ...
        <div v-if="member.id !== currentUser?.id" class="flex shrink-0 items-center gap-1"><UButton v-if="member.status !== 'archived'" color="neutral" variant="ghost" icon="i-lucide-archive" :aria-label="t('users.archiveAction')" class="min-h-11 min-w-11 active:scale-[0.96] transition-transform" :loading="archivePending" @click="archive(member)" /><UButton v-else color="error" variant="ghost" icon="i-lucide-trash-2" :aria-label="t('users.deleteAction')" class="min-h-11 min-w-11 active:scale-[0.96] transition-transform" @click="askToDelete(member)" /></div>
      </div>
    </div>
    ...
    <UModal v-model:open="deleteOpen" :title="t('users.deleteAction')"><template #body><div class="space-y-5"><UAlert color="error" variant="soft" :title="t('common.irreversible')" :description="t('common.irreversible')" /><p>{{ t('users.name') }}: <strong>{{ memberToDelete?.name }}</strong></p><UInput v-model="confirmationName" class="w-full" :placeholder="memberToDelete?.name" /><UAlert v-if="error" color="error" variant="soft" :description="error" /><div class="form-actions"><UButton color="neutral" variant="ghost" @click="deleteOpen = false">{{ t('users.cancel') }}</UButton><UButton color="error" :loading="deletePending" :disabled="confirmationName !== memberToDelete?.name" @click="deletePermanently">{{ t('users.deleteAction') }}</UButton></div></div></template></UModal>
  </section>
</template>

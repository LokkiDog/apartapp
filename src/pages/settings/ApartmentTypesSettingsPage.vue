...
<template>
  <section class="page-wrap space-y-6">
    ...
    <USlideover v-model:open="open" :title="typeToEdit ? t('apartmentTypes.edit') : t('apartmentTypes.new')">
      <template #body>
        <form id="apartment-type-form" class="form-grid" @submit.prevent="save">
          <UFormField :label="t('apartmentTypes.name')"><UInput v-model="form.name" placeholder="Studio" required /></UFormField>
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-3"><UFormField :label="t('apartmentTypes.cleaning')"><MoneyInput v-model="form.cleanerPoolEur" :empty-value="0" required /></UFormField><UFormField :label="t('apartmentTypes.laundry')"><MoneyInput v-model="form.laundryEur" :empty-value="0" required /></UFormField><UFormField :label="t('apartmentTypes.service')"><MoneyInput v-model="form.serviceEur" :empty-value="0" required /></UFormField></div>
          <section><div class="mb-2 flex items-start justify-between gap-3"><div><p class="font-semibold">{{ t('common.checklist') }}</p><p class="text-sm text-[var(--color-muted)]">{{ t('common.checklistHint') }}</p></div><UButton type="button" color="neutral" variant="ghost" size="sm" icon="i-lucide-plus" @click="addChecklistItem">{{ t('common.add') }}</UButton></div><div class="space-y-2"><div v-for="(item, index) in form.defaultChecklist" :key="index" class="checklist-edit-row"><span class="grid size-8 shrink-0 place-items-center text-sm tabular-nums text-[var(--color-muted)]">{{ index + 1 }}</span><UInput v-model="form.defaultChecklist[index]" class="min-w-0 flex-1" :placeholder="t('common.action')" required /><div class="checklist-edit-actions"><UButton type="button" color="neutral" variant="ghost" size="sm" icon="i-lucide-chevron-up" :disabled="index === 0" :aria-label="t('common.action')" @click="moveChecklistItem(index, -1)" /><UButton type="button" color="neutral" variant="ghost" size="sm" icon="i-lucide-chevron-down" :disabled="index === form.defaultChecklist.length - 1" :aria-label="t('common.action')" @click="moveChecklistItem(index, 1)" /><UButton type="button" color="neutral" variant="ghost" size="sm" icon="i-lucide-x" :aria-label="t('common.removeItem')" @click="removeChecklistItem(index)" /></div></div></div></section>
          <p class="text-sm text-[var(--color-muted)]">{{ t('apartmentTypes.checklistNote') }}</p>
          <section class="border-t border-[var(--color-line)] pt-4"><div class="mb-2"><p class="font-semibold">{{ t('apartmentTypes.autoWriteOff') }}</p><p class="text-sm text-[var(--color-muted)]">{{ t('apartmentTypes.autoWriteOffHint') }}</p></div><div v-if="consumables?.length" class="divide-y divide-[var(--color-line)] rounded-xl border border-[var(--color-line)]"><div v-for="item in consumables" :key="item.id" class="flex min-h-14 items-center gap-3 px-3 py-2"><UCheckbox :model-value="Boolean(writeOffRule(item.id))" :label="item.name" class="min-w-0 flex-1" @update:model-value="setWriteOffEnabled(item.id, Boolean($event))" /><UInput v-if="writeOffRule(item.id)" v-model.number="writeOffRule(item.id)!.quantity" type="number" min=".001" step=".001" class="w-28" :aria-label="`${item.name}: ${t('common.quantityPerCleaning')}`"><template #trailing>{{ item.unit }}</template></UInput></div></div><p v-else class="text-sm text-[var(--color-muted)]">{{ t('apartmentTypes.autoWriteOffNoConsumables') }}</p></section>

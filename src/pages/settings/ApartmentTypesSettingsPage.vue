...
type ApartmentType = { id: string; name: string; ownerTotalEur: number; cleanerPoolEur: number; laundryEur: number; serviceEur: number; defaultLinenGuestCount: number; defaultChecklist: string[]; autoWriteOffs: AutoWriteOff[] }
...
const form = reactive({ name: '', defaultLinenGuestCount: 2, cleanerPoolEur: 0 as number | null, laundryEur: 0 as number | null, serviceEur: 0 as number | null, defaultChecklist: [] as string[], autoWriteOffs: [] as AutoWriteOff[] })
...
function resetForm() { Object.assign(form, { name: '', defaultLinenGuestCount: 2, cleanerPoolEur: 0, laundryEur: 0, serviceEur: 0, defaultChecklist: [...checklistTemplate.value], autoWriteOffs: [] }) }
function openCreate() { typeToEdit.value = null; resetForm(); validation.reset(); error.value = ''; open.value = true }
function openEdit(type: ApartmentType) { typeToEdit.value = type; Object.assign(form, { name: type.name, defaultLinenGuestCount: type.defaultLinenGuestCount, cleanerPoolEur: type.cleanerPoolEur, laundryEur: type.laundryEur, serviceEur: type.serviceEur, defaultChecklist: [...(type.defaultChecklist ?? checklistTemplate.value)], autoWriteOffs: (type.autoWriteOffs ?? []).map(rule => ({ ...rule, quantity: Number(rule.quantity) })) }); validation.reset(); error.value = ''; open.value = true }
  ...
  <section class="page-wrap space-y-6">
    ...
    <div v-else-if="types?.length" class="grid gap-4 md:grid-cols-2">
      <UCard v-for="type in types" :key="type.id">
        ...
        <dl class="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4"><div><dt class="text-[var(--color-muted)]">{{ t('apartmentTypes.cleaning') }}</dt><dd class="mt-1 font-semibold tabular-nums">{{ formatEuro(type.cleanerPoolEur) }}</dd></div><div><dt class="text-[var(--color-muted)]">{{ t('apartmentTypes.laundry') }}</dt><dd class="mt-1 font-semibold tabular-nums">{{ formatEuro(type.laundryEur) }}</dd></div><div><dt class="text-[var(--color-muted)]">{{ t('apartmentTypes.service') }}</dt><dd class="mt-1 font-semibold tabular-nums">{{ formatEuro(type.serviceEur) }}</dd></div><div><dt class="text-[var(--color-muted)]">{{ t('apartmentTypes.defaultLinenGuests') }}</dt><dd class="mt-1 font-semibold tabular-nums">{{ type.defaultLinenGuestCount }}</dd></div></dl>
    ...
    <USlideover v-model:open="open" :title="typeToEdit ? t('apartmentTypes.edit') : t('apartmentTypes.new')" :modal="true" :overlay="true">
      <template #body>
        <UForm :key="validation.formKey.value" id="apartment-type-form" :state="form" :validate="validate" :validate-on="validation.validateOn.value" novalidate class="form-grid" @error="validation.onError" @submit="save">
          ...
          <UFormField name="defaultLinenGuestCount" :label="t('apartmentTypes.defaultLinenGuests')" :description="t('apartmentTypes.defaultLinenGuestsHint')"><UInput v-model.number="form.defaultLinenGuestCount" type="number" min="1" max="50" class="w-full tabular-nums" /></UFormField>
      ...
      <template #footer><div class="form-actions form-actions--footer"><UButton type="button" color="neutral" variant="ghost" @click="open = false">{{ t('common.cancel') }}</UButton><UButton type="submit" form="apartment-type-form" :loading="pending">{{ typeToEdit ? t('common.save') : t('apartmentTypes.create') }}</UButton></div></template>
    </USlideover>

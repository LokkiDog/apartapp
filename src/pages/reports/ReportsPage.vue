  ...
  <section class="page-wrap space-y-6">
    ...
    <template v-if="report">
      ...
      <div v-else-if="activeTab === 'workload'" class="space-y-5">
        <div class="grid gap-4 sm:grid-cols-3"><MetricTile label="Без даты" :value="report.workload.unscheduledCleanings.length" icon="i-lucide-calendar-x" :tone="report.workload.unscheduledCleanings.length ? 'warning' : 'neutral'" /><MetricTile label="Просроченные задачи" :value="report.workload.overdueTasks.length" icon="i-lucide-clock-alert" :tone="report.workload.overdueTasks.length ? 'danger' : 'neutral'" /><MetricTile label="Задачи без срока" :value="report.workload.undatedTasks.length" icon="i-lucide-list-todo" /></div>
        ...
        <section v-if="report.workload.unscheduledCleanings.length || report.workload.overdueTasks.length || report.workload.undatedTasks.length" class="surface p-5 sm:p-6"><h2 class="text-lg font-semibold">Требует планирования</h2><div class="mt-4 grid gap-3 lg:grid-cols-3"><div v-if="report.workload.unscheduledCleanings.length" class="report-attention"><strong>Уборки без даты</strong><p v-for="item in report.workload.unscheduledCleanings" :key="item.id">{{ item.apartmentName }} · {{ item.hotelName }}</p></div><div v-if="report.workload.overdueTasks.length" class="report-attention report-attention--danger"><strong>Просроченные задачи</strong><p v-for="item in report.workload.overdueTasks" :key="item.id">{{ item.title }} · {{ item.apartmentName }}</p></div><div v-if="report.workload.undatedTasks.length" class="report-attention"><strong>Задачи без срока</strong><p v-for="item in report.workload.undatedTasks" :key="item.id">{{ item.title }} · {{ item.apartmentName }}</p></div></div></section>
      </div>
      ...
      <div v-else class="space-y-6">
        ...
        <section class="surface overflow-hidden"><div class="report-section-heading"><div><h2>Все финансовые операции</h2><p>{{ report.finance.entries.length }} строк за выбранный период</p></div></div><div v-if="report.finance.entries.length" class="report-finance-table"><div class="report-finance-head"><span>Дата и категория</span><span>Объект</span><span>Управляющий</span><span>Сумма</span></div><div v-for="entry in report.finance.entries" :key="entry.id" class="report-finance-row"><div><p class="font-medium">{{ formatDate(entry.occurredOn) }}</p><p>{{ financeTypeLabels[entry.type] ?? entry.type }}</p></div><div><p class="font-medium">{{ entry.apartmentName }}</p><p>{{ entry.hotelName }}</p></div><div><p class="font-medium">{{ entry.managerName }}</p><p>{{ entry.description }}</p></div><strong>{{ formatEuro(entry.amountEur) }}</strong></div></div><EmptyState v-else icon="i-lucide-receipt" title="Операций нет" description="В выбранном периоде финансовых записей нет." /></section>
      </div>
    </template>
  </section>
</template>

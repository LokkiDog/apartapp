...
const priorityLabels = computed<Record<string, string>>(() => ({ low: t('work.priorityLow'), normal: t('work.priorityNormal'), high: t('work.priorityHigh'), urgent: t('work.priorityUrgent') }))
...
function cleaningAmountLabel(cleaning: Cleaning) { return cleaning.tariffSnapshot.ownerTotalEur !== undefined ? t('work.cost') : t('work.pool') }
...
function cleanerNames(cleaning: Cleaning) { return cleaning.assignments.map(item => item.cleaner.name).join(', ') || t('work.notAssigned') }
...
function cleaningMenuItems(cleaning: Cleaning): DropdownMenuItem[] {
  const items: DropdownMenuItem[] = [{ label: `${t('common.open')} ${t('work.cleanings').toLocaleLowerCase()}`, icon: 'i-lucide-arrow-up-right', onSelect: () => { void navigateTo(workHref('cleaning', cleaning.id)) } }]
  if (isAdministrator.value && !['completed', 'canceled'].includes(cleaning.status)) items.push({ label: !cleaning.assignments.length ? t('common.assignCleaning') : t('work.editCleaning'), icon: !cleaning.assignments.length ? 'i-lucide-calendar-plus' : 'i-lucide-pencil', onSelect: () => openEditCleaning(cleaning) })
  if (isAdministrator.value) items.push({ label: t('work.delete'), icon: 'i-lucide-trash-2', color: 'error', onSelect: () => askToDelete('cleaning', cleaning) })
  return items
}

function taskMenuItems(task: Task): DropdownMenuItem[] {
  const items: DropdownMenuItem[] = [{ label: `${t('common.open')} ${t('work.tasks').toLocaleLowerCase()}`, icon: 'i-lucide-arrow-up-right', onSelect: () => { void navigateTo(workHref('task', task.id)) } }]
  if (isAdministrator.value && !['completed', 'canceled'].includes(task.status)) items.push({ label: t('work.cancelTask'), icon: 'i-lucide-ban', color: 'error', onSelect: () => { void cancelTask(task.id) } })
  if (isAdministrator.value && !['completed', 'canceled'].includes(task.status)) items.push({ label: t('work.editTask'), icon: 'i-lucide-pencil', onSelect: () => openEditTask(task) })
  if (isAdministrator.value) items.push({ label: t('work.delete'), icon: 'i-lucide-trash-2', color: 'error', onSelect: () => askToDelete('task', task) })
  return items
}
</script>
...
<template>
  <section class="page-wrap space-y-6">
    ...
    <div class="work-tab-controls">
      <UFieldGroup class="work-tab-switch"><UButton :variant="tab === 'cleanings' ? 'solid' : 'soft'" @click="tab = 'cleanings'">{{ t('work.cleanings') }} <UBadge color="neutral" variant="soft">{{ cleanings?.length ?? 0 }}</UBadge></UButton><UButton :variant="tab === 'tasks' ? 'solid' : 'soft'" @click="tab = 'tasks'">{{ t('work.tasks') }} <UBadge color="neutral" variant="soft">{{ tasks?.length ?? 0 }}</UBadge></UButton></UFieldGroup>
      <UButton v-if="isAdministrator" class="work-create-button" :icon="tab === 'cleanings' ? 'i-lucide-sparkles' : 'i-lucide-plus'" :aria-label="tab === 'cleanings' ? t('work.newCleaning') : t('work.newTask')" @click="tab === 'cleanings' ? openCreateCleaning() : openCreateTask()"><span class="work-create-button__label">{{ tab === 'cleanings' ? t('work.newCleaning') : t('work.newTask') }}</span></UButton>
    </div>

    <template v-if="tab === 'cleanings'">
      <div class="work-planning-controls">
        <UFieldGroup v-if="isAdministrator" class="work-planning-switch"><UButton :variant="planningMode === 'days' ? 'solid' : 'soft'" icon="i-lucide-calendar-days" @click="planningMode = 'days'">{{ t('work.days') }}</UButton><UButton :variant="planningMode === 'cleaners' ? 'solid' : 'soft'" icon="i-lucide-users" @click="planningMode = 'cleaners'">{{ t('work.cleaners') }}</UButton><UButton :variant="planningMode === 'apartments' ? 'solid' : 'soft'" icon="i-lucide-building-2" @click="planningMode = 'apartments'">{{ t('work.apartments') }}</UButton></UFieldGroup>
        <UButton class="work-history-button" color="neutral" variant="ghost" icon="i-lucide-history" @click="historyOpen = !historyOpen">{{ historyOpen ? `${t('common.hide')} ${t('common.history').toLocaleLowerCase()}` : t('common.history') }} <UBadge color="neutral" variant="soft">{{ cleaningPlan.history.length }}</UBadge></UButton>
      </div>

      <section v-if="cleaningPlan.attention.length" class="surface overflow-hidden">
        <div class="flex items-center gap-3 border-b border-[var(--color-line)] bg-amber-50/70 px-5 py-4"><UIcon name="i-lucide-calendar-clock" class="size-5 text-amber-700" /><div><h2 class="font-semibold">{{ t('reports.attention') }}</h2><p class="text-sm text-amber-800/80">{{ t('reports.refreshSummary') }}</p></div></div>
        <div class="divide-y divide-[var(--color-line)] px-5 sm:px-6"><article v-for="cleaning in cleaningPlan.attention" :key="`attention-${cleaning.id}`" class="flex items-center gap-3 py-4" @click="openCleaningCard($event, cleaning)"><div class="grid size-9 shrink-0 place-items-center rounded-lg bg-[var(--color-primary-soft)] text-[var(--color-primary)]"><UIcon name="i-lucide-sparkles" class="size-4" /></div><div class="min-w-0 flex-1"><p class="truncate font-medium">{{ cleaning.apartment.name }}</p><p class="truncate text-sm text-[var(--color-muted)]">{{ cleaningSubtitle(cleaning) }} · {{ formatDate(cleaning.scheduledOn) }} · {{ cleanerNames(cleaning) }}</p></div><StatusBadge :label="t('work.statusUnassigned')" tone="warning" /><UButton v-if="isAdministrator" color="primary" variant="soft" icon="i-lucide-calendar-plus" @click="openEditCleaning(cleaning)">{{ t('common.assignCleaning') }}</UButton><UDropdownMenu v-if="cleaningMenuItems(cleaning).length" :items="cleaningMenuItems(cleaning)" :content="{ align: 'end' }"><UButton color="neutral" variant="ghost" icon="i-lucide-ellipsis-vertical" :aria-label="t('work.cleanings')" class="min-h-11 min-w-11 active:scale-[0.96] transition-transform" /></UDropdownMenu></article></div>
      </section>

      <template v-if="planningMode === 'days'">
        <section v-for="day in cleaningPlan.days" :key="day.date" class="surface overflow-hidden">
          <div class="flex items-center justify-between gap-3 border-b border-[var(--color-line)] px-5 py-4 sm:px-6"><div><h2 class="font-semibold">{{ day.date === today ? t('common.today') : formatDate(day.date) }}</h2><p class="text-sm text-[var(--color-muted)]">{{ day.cleanings.filter(item => !isFinished(item)).length }} {{ t('work.cleanings').toLocaleLowerCase() }}</p></div><UButton v-if="day.cleanings.some(item => isFinished(item))" color="neutral" variant="ghost" size="sm" :icon="isFinishedDay(day.date) ? 'i-lucide-chevron-down' : 'i-lucide-chevron-up'" @click="toggleFinishedDay(day.date)">{{ isFinishedDay(day.date) ? t('common.show') : t('common.hide') }}</UButton></div>
          <div v-for="route in routesForDay(day.cleanings, !isFinishedDay(day.date))" :key="`${day.date}-${route.cleanerId}`" class="border-b border-[var(--color-line)] last:border-b-0"><div class="flex items-center gap-2 bg-[var(--color-surface-muted)] px-5 py-2.5 text-sm font-semibold sm:px-6"><UIcon name="i-lucide-user-round" class="size-4 text-[var(--color-primary)]" />{{ route.cleanerName }}<span class="ml-auto text-xs font-normal text-[var(--color-muted)]">{{ route.cleanings.length }} {{ t('workExtra.cleaningsCount') }}</span></div><div class="divide-y divide-[var(--color-line)] px-5 sm:px-6"><article v-for="cleaning in route.cleanings" :key="`${day.date}-${route.cleanerId}-${cleaning.id}`" class="group flex items-center gap-3 py-4" :draggable="isAdministrator && !isFinished(cleaning)" @dragstart="startDragging($event, cleaning.id)" @dragover.prevent @drop="dropRouteItem($event, day.date, route.cleanerId, cleaning)" @click="openCleaningCard($event, cleaning)"><span class="grid size-8 shrink-0 place-items-center rounded-lg bg-[var(--color-primary-soft)] text-xs font-semibold tabular-nums text-[var(--color-primary)]">{{ isFinished(cleaning) ? '✓' : routeLabel(routeIndex(day.cleanings, route.cleanerId, cleaning)) }}</span><div class="min-w-0 flex-1"><p class="truncate font-semibold">{{ cleaning.apartment.name }}</p><p class="truncate text-sm text-[var(--color-muted)]">{{ cleaningSubtitle(cleaning) }}</p><p v-if="cleaning.hasProblem" class="mt-1 truncate text-sm text-red-700">{{ cleaning.problemDescription }}</p></div><UButton v-if="canOperate('cleaning', cleaning) && ['assigned', 'in_progress'].includes(cleaning.status)" color="primary" variant="soft" size="sm" icon="i-lucide-circle-check" @click="askQuickComplete('cleaning', cleaning)">{{ t('work.complete') }}</UButton><StatusBadge :label="statusLabels[cleaning.status] ?? cleaning.status" :tone="statusTones[cleaning.status] ?? 'neutral'" /><div v-if="isAdministrator && !isFinished(cleaning)" class="hidden items-center gap-0.5 sm:flex"><UButton color="neutral" variant="ghost" size="xs" icon="i-lucide-chevron-up" :disabled="routeIndex(day.cleanings, route.cleanerId, cleaning) <= 0" :aria-label="t('workExtra.moveUp')" @click="moveRouteItem(day.date, route.cleanerId, cleaning, -1)" /><UButton color="neutral" variant="ghost" size="xs" icon="i-lucide-chevron-down" :disabled="routeIndex(day.cleanings, route.cleanerId, cleaning) >= activeRouteItems(day.cleanings, route.cleanerId).length - 1" :aria-label="t('workExtra.moveDown')" @click="moveRouteItem(day.date, route.cleanerId, cleaning, 1)" /></div><UDropdownMenu v-if="cleaningMenuItems(cleaning).length" :items="cleaningMenuItems(cleaning)" :content="{ align: 'end' }"><UButton color="neutral" variant="ghost" icon="i-lucide-ellipsis-vertical" :aria-label="t('workExtra.cleaningActions')" class="min-h-11 min-w-11 active:scale-[0.96] transition-transform" /></UDropdownMenu></article></div></div>
        </section>
      </template>

      <template v-else-if="planningMode === 'cleaners'">
        <section v-for="employee in cleanerPlan" :key="employee.cleanerId" class="surface overflow-hidden"><div class="flex items-center gap-3 border-b border-[var(--color-line)] px-5 py-4 sm:px-6"><div class="grid size-10 place-items-center rounded-full bg-[var(--color-primary-soft)] text-[var(--color-primary)]"><UIcon name="i-lucide-user-round" class="size-5" /></div><div><h2 class="font-semibold">{{ employee.cleanerName }}</h2><p class="text-sm text-[var(--color-muted)]">{{ employee.days.reduce((total, day) => total + day.cleanings.filter(item => !isFinished(item)).length, 0) }} {{ t('workExtra.activeCleanings') }}</p></div></div><div v-for="day in employee.days" :key="`${employee.cleanerId}-${day.date}`" class="border-b border-[var(--color-line)] last:border-b-0"><div class="bg-[var(--color-surface-muted)] px-5 py-2.5 text-sm font-semibold sm:px-6">{{ day.date === today ? t('common.today') : formatDate(day.date) }}</div><div class="divide-y divide-[var(--color-line)] px-5 sm:px-6"><article v-for="cleaning in day.cleanings" :key="`${employee.cleanerId}-${day.date}-${cleaning.id}`" class="group flex items-center gap-3 py-4" :draggable="isAdministrator && !isFinished(cleaning)" @dragstart="startDragging($event, cleaning.id)" @dragover.prevent @drop="dropRouteItem($event, day.date, employee.cleanerId, cleaning)" @click="openCleaningCard($event, cleaning)"><span class="grid size-8 shrink-0 place-items-center rounded-lg bg-[var(--color-primary-soft)] text-xs font-semibold tabular-nums text-[var(--color-primary)]">{{ isFinished(cleaning) ? '✓' : routeLabel(routeIndex(day.cleanings, employee.cleanerId, cleaning)) }}</span><div class="min-w-0 flex-1"><p class="truncate font-semibold">{{ cleaning.apartment.name }}</p><p class="truncate text-sm text-[var(--color-muted)]">{{ cleaningSubtitle(cleaning) }}</p></div><UButton v-if="canOperate('cleaning', cleaning) && ['assigned', 'in_progress'].includes(cleaning.status)" color="primary" variant="soft" size="sm" icon="i-lucide-circle-check" @click="askQuickComplete('cleaning', cleaning)">{{ t('work.complete') }}</UButton><StatusBadge :label="statusLabels[cleaning.status] ?? cleaning.status" :tone="statusTones[cleaning.status] ?? 'neutral'" /><div v-if="isAdministrator && !isFinished(cleaning)" class="hidden items-center gap-0.5 sm:flex"><UButton color="neutral" variant="ghost" size="xs" icon="i-lucide-chevron-up" :disabled="routeIndex(day.cleanings, employee.cleanerId, cleaning) <= 0" :aria-label="t('workExtra.moveUp')" @click="moveRouteItem(day.date, employee.cleanerId, cleaning, -1)" /><UButton color="neutral" variant="ghost" size="xs" icon="i-lucide-chevron-down" :disabled="routeIndex(day.cleanings, employee.cleanerId, cleaning) >= activeRouteItems(day.cleanings, employee.cleanerId).length - 1" :aria-label="t('workExtra.moveDown')" @click="moveRouteItem(day.date, employee.cleanerId, cleaning, 1)" /></div><UDropdownMenu v-if="cleaningMenuItems(cleaning).length" :items="cleaningMenuItems(cleaning)" :content="{ align: 'end' }"><UButton color="neutral" variant="ghost" icon="i-lucide-ellipsis-vertical" :aria-label="t('workExtra.cleaningActions')" class="min-h-11 min-w-11 active:scale-[0.96] transition-transform" /></UDropdownMenu></article></div></div></section>
      </template>
      ...
      <template v-else>
        <section v-for="group in apartmentPlan" :key="group.apartmentId" class="surface overflow-hidden">
          ...
          <div v-for="day in group.days" :key="`${group.apartmentId}-${day.date}`" class="border-b border-[var(--color-line)] last:border-b-0">
            <div class="flex items-center justify-between gap-3 bg-[var(--color-surface-muted)] px-5 py-2.5 text-sm font-semibold sm:px-6"><span>{{ day.date === today ? t('common.today') : formatDate(day.date) }}</span><UButton v-if="day.cleanings.some(item => isFinished(item))" color="neutral" variant="ghost" size="xs" :icon="isFinishedDay(day.date) ? 'i-lucide-chevron-down' : 'i-lucide-chevron-up'" @click="toggleFinishedDay(day.date)">{{ isFinishedDay(day.date) ? t('workExtra.showCompleted') : `${day.cleanings.filter(item => !isFinished(item)).length} ${t('workExtra.activeCleanings')}` }}</UButton><span v-else class="text-xs font-normal text-[var(--color-muted)]">{{ day.cleanings.length }} {{ t('workExtra.cleaningsCount') }}</span></div>
            <div class="divide-y divide-[var(--color-line)] px-5 sm:px-6">
              <article v-for="cleaning in day.cleanings.filter(item => !isFinishedDay(day.date) || !isFinished(item))" @click="openCleaningCard($event, cleaning)" :key="`${group.apartmentId}-${day.date}-${cleaning.id}`" class="group flex items-center gap-3 py-4">
                ...
                <UDropdownMenu v-if="cleaningMenuItems(cleaning).length" :items="cleaningMenuItems(cleaning)" :content="{ align: 'end' }"><UButton color="neutral" variant="ghost" icon="i-lucide-ellipsis-vertical" :aria-label="t('workExtra.cleaningActions')" class="min-h-11 min-w-11 active:scale-[0.96] transition-transform" /></UDropdownMenu>
              </article>
            </div>
          </div>
        </section>
      ...
      <section v-if="cleaningPlan.later.length" class="surface overflow-hidden"><button type="button" class="flex w-full items-center justify-between px-5 py-4 text-left font-semibold sm:px-6" @click="laterOpen = !laterOpen"><span class="flex items-center gap-2"><UIcon name="i-lucide-calendar-plus" class="size-5 text-[var(--color-primary)]" />{{ t('workExtra.later') }} <UBadge color="neutral" variant="soft">{{ cleaningPlan.later.length }}</UBadge></span><UIcon :name="laterOpen ? 'i-lucide-chevron-up' : 'i-lucide-chevron-down'" class="size-5" /></button><div v-if="laterOpen" class="divide-y divide-[var(--color-line)] border-t border-[var(--color-line)] px-5 sm:px-6"><article v-for="cleaning in cleaningPlan.later" :key="`later-${cleaning.id}`" class="flex items-center gap-3 py-4" @click="openCleaningCard($event, cleaning)"><span class="text-sm font-medium tabular-nums text-[var(--color-muted)]">{{ formatDate(cleaning.scheduledOn) }}</span><div class="min-w-0 flex-1"><p class="truncate font-semibold">{{ cleaning.apartment.name }}</p><p class="truncate text-sm text-[var(--color-muted)]">{{ cleaningSubtitle(cleaning) }} · {{ cleanerNames(cleaning) }}</p></div><UButton v-if="canOperate('cleaning', cleaning) && ['assigned', 'in_progress'].includes(cleaning.status)" color="primary" variant="soft" size="sm" icon="i-lucide-circle-check" @click="askQuickComplete('cleaning', cleaning)">{{ t('work.complete') }}</UButton><StatusBadge :label="statusLabels[cleaning.status] ?? cleaning.status" :tone="statusTones[cleaning.status] ?? 'neutral'" /></article></div></section>

      <section v-if="historyOpen" class="surface overflow-hidden"><div class="border-b border-[var(--color-line)] px-5 py-4 sm:px-6"><h2 class="font-semibold">{{ t('workExtra.historyTitle') }}</h2><p class="text-sm text-[var(--color-muted)]">{{ t('workExtra.historyDescription') }}</p></div><div v-if="cleaningPlan.history.length" class="divide-y divide-[var(--color-line)] px-5 sm:px-6"><article v-for="cleaning in cleaningPlan.history" :key="`history-${cleaning.id}`" class="flex items-center gap-3 py-4" @click="openCleaningCard($event, cleaning)"><span class="text-sm font-medium tabular-nums text-[var(--color-muted)]">{{ formatDate(cleaning.scheduledOn) }}</span><div class="min-w-0 flex-1"><p class="truncate font-semibold">{{ cleaning.apartment.name }}</p><p class="truncate text-sm text-[var(--color-muted)]">{{ cleaningSubtitle(cleaning) }} · {{ cleanerNames(cleaning) }}</p></div><UButton v-if="canOperate('cleaning', cleaning) && ['assigned', 'in_progress'].includes(cleaning.status)" color="primary" variant="soft" size="sm" icon="i-lucide-circle-check" @click="askQuickComplete('cleaning', cleaning)">{{ t('work.complete') }}</UButton><StatusBadge :label="statusLabels[cleaning.status] ?? cleaning.status" :tone="statusTones[cleaning.status] ?? 'neutral'" /></article></div><p v-else class="px-5 py-6 text-sm text-[var(--color-muted)] sm:px-6">{{ t('workExtra.historyEmpty') }}</p></section>
      <EmptyState v-if="!cleanings?.length" icon="i-lucide-sparkles" :title="t('workExtra.noCleanings')" :description="t('workExtra.noCleaningsDescription')" />
    </template>

    <template v-if="tab === 'tasks'">
      <div v-if="tasks?.length" class="surface divide-y divide-[var(--color-line)] px-5 sm:px-6"><article v-for="task in tasks" :key="task.id" class="flex items-center gap-3 py-4" @click="openTaskCard($event, task)"><div class="grid size-10 shrink-0 place-items-center rounded-xl bg-[#edf3f7] text-[#356882]"><UIcon name="i-lucide-clipboard-check" class="size-5" /></div><div class="min-w-0 flex-1"><NuxtLink :to="workHref('task', task.id)" class="block rounded-lg p-1 -m-1 hover:bg-[var(--color-surface-muted)]"><p class="truncate font-semibold">{{ task.title }}</p><p class="mt-1 truncate text-sm text-[var(--color-muted)]">{{ task.dueOn ? formatDate(task.dueOn) : t('work.noDeadline') }} · {{ task.assignee?.name ?? t('work.notAssigned') }}</p><p class="truncate text-sm text-[var(--color-muted)]">{{ task.apartment.name }} · {{ task.apartment.hotel.name }}</p></NuxtLink></div><UButton v-if="canOperate('task', task) && ['open', 'in_progress'].includes(task.status)" color="primary" variant="soft" size="sm" icon="i-lucide-circle-check" @click="askQuickComplete('task', task)">{{ t('work.complete') }}</UButton><StatusBadge :label="priorityLabels[task.priority] ?? task.priority" :tone="task.priority === 'urgent' ? 'danger' : task.priority === 'high' ? 'warning' : 'neutral'" /><StatusBadge :label="statusLabels[task.status] ?? task.status" :tone="statusTones[task.status] ?? 'neutral'" /><UDropdownMenu v-if="taskMenuItems(task).length" :items="taskMenuItems(task)" :content="{ align: 'end' }"><UButton color="neutral" variant="ghost" icon="i-lucide-ellipsis-vertical" :aria-label="t('workExtra.taskActions')" class="min-h-11 min-w-11 active:scale-[0.96] transition-transform" /></UDropdownMenu><UAlert v-if="task.hasProblem" class="mt-3" color="error" variant="soft" icon="i-lucide-circle-alert" :title="t('workExtra.problem')" :description="task.problemDescription" /></article></div>
      <EmptyState v-else icon="i-lucide-clipboard-check" :title="t('workExtra.noTasks')" :description="t('workExtra.noTasksDescription')"><template #actions><UButton v-if="isAdministrator" @click="openCreateTask">{{ t('work.newTask') }}</UButton></template></EmptyState>
    </template>

    <USlideover v-model:open="taskOpen" :title="editingTask ? t('workExtra.editTask') : t('workExtra.newTask')"><template #body><form id="task-form" class="form-grid" @submit.prevent="saveTask"><UFormField :label="t('work.apartment')" :help="editingTask ? t('workExtra.taskEditHint') : undefined"><USelect v-model="taskForm.apartmentId" :items="(apartments ?? []).map(apartment => ({ label: `${apartment.name} · ${apartment.hotel.name}`, value: apartment.id }))" class="w-full" :disabled="Boolean(editingTask && editingTask.status !== 'open')" required /></UFormField><UFormField :label="t('workExtra.taskTitle')"><UInput v-model="taskForm.title" required /></UFormField><UFormField :label="t('workExtra.taskDescription')"><UTextarea v-model="taskForm.description" /></UFormField><UFormField :label="t('workExtra.taskPriority')"><USelect v-model="taskForm.priority" :items="Object.entries(priorityLabels).map(([value, label]) => ({ value, label }))" class="w-full" /></UFormField><UFormField :label="t('work.assignee')"><USelect v-model="taskForm.assigneeId" :items="[{ label: t('work.notAssigned'), value: 'unassigned' }, ...(team ?? []).filter(member => member.roles.includes('cleaner')).map(member => ({ label: member.name, value: member.id }))]" class="w-full" /></UFormField><UFormField v-if="isAdministrator" :label="t('workExtra.ownerCost')"><MoneyInput v-model="taskForm.ownerCostEur" /></UFormField><UFormField :label="t('workExtra.dueDate')"><DateInput v-model="taskForm.dueOn" /></UFormField><UAlert v-if="error" color="error" variant="soft" :description="error" /></form></template><template #footer><div class="form-actions form-actions--footer"><UButton type="button" color="neutral" variant="ghost" @click="taskOpen = false">{{ t('work.cancel') }}</UButton><UButton type="submit" form="task-form" :loading="pending">{{ editingTask ? t('common.save') : t('workExtra.newTask') }}</UButton></div></template></USlideover>
    ...
    <USlideover v-model:open="stockOpen" :title="t('work.stockTitle')"><template #body><form id="stock-usage-form" class="form-grid" @submit.prevent="recordUsage"><UFormField :label="t('work.consumable')"><USelect v-model="usageForm.consumableId" :items="stockItems.filter(item => item.quantity > 0).map(item => ({ label: `${item.consumable.name} · ${item.quantity} ${item.consumable.unit}`, value: item.consumable.id }))" class="w-full" required /></UFormField><UFormField :label="t('work.quantity')"><UInput v-model.number="usageForm.quantity" type="number" min=".001" step=".001" required /></UFormField><UFormField :label="t('workExtra.comment')"><UInput v-model="usageForm.note" /></UFormField><UAlert v-if="error" color="error" variant="soft" :description="error" /></form></template><template #footer><div class="form-actions form-actions--footer"><UButton type="button" color="neutral" variant="ghost" @click="stockOpen = false">{{ t('work.cancel') }}</UButton><UButton type="submit" form="stock-usage-form" :loading="pending">{{ t('work.writeOff') }}</UButton></div></template></USlideover>

    <USlideover v-model:open="completeOpen" :title="inventoryOnly ? t('work.stockTitle') : t('work.complete')"><template #body><form v-if="selected" id="complete-work-form" class="form-grid" @submit.prevent="inventoryOnly ? saveInventory() : complete()"><section v-if="selected.kind === 'cleaning'" class="rounded-2xl border border-[var(--color-line)] bg-[var(--color-surface-muted)] p-4"><details open><summary class="cursor-pointer list-none font-semibold"><span class="flex items-center justify-between gap-3">{{ t('work.stockTitle') }} <UIcon name="i-lucide-chevron-down" class="size-4 text-[var(--color-muted)]" /></span><p class="mt-1 text-sm font-normal text-[var(--color-muted)]">{{ t('workExtra.stockHint') }}</p></summary><div class="mt-4 space-y-3"><div v-for="item in inventoryReports" :key="item.consumable.id" class="rounded-xl bg-white p-3 shadow-sm"><div class="flex items-start justify-between gap-3"><div><p class="font-medium">{{ item.consumable.name }}</p><p class="text-xs text-[var(--color-muted)]">{{ t('workExtra.stockNow') }} {{ item.quantity }} {{ item.consumable.unit }}<span v-if="item.consumable.autoWriteOffEnabled"> · {{ t('workExtra.autoWriteOff') }} {{ item.consumable.autoWriteOffQuantity }} {{ item.consumable.unit }}</span></p></div><UIcon name="i-lucide-package" class="mt-0.5 size-4 text-[var(--color-primary)]" /></div><div class="mt-3 grid grid-cols-2 gap-3"><UFormField :label="t('workExtra.usedQuantity')"><UInput v-model.number="item.usedQuantity" type="number" min="0" step=".001" @update:model-value="syncExpectedRemaining(item)"><template #trailing>{{ item.consumable.unit }}</template></UInput></UFormField><UFormField :label="t('workExtra.remainingQuantity')"><UInput v-model.number="item.remainingQuantity" type="number" min="0" step=".001" @update:model-value="item.remainingTouched = true"><template #trailing>{{ item.consumable.unit }}</template></UInput></UFormField></div><p v-if="item.discrepancyQuantity" class="mt-2 text-xs text-amber-700">{{ t('workExtra.discrepancy') }} {{ item.discrepancyQuantity > 0 ? '+' : '' }}{{ item.discrepancyQuantity }} {{ item.consumable.unit }}</p></div><p v-if="!inventoryReports.length" class="text-sm text-[var(--color-muted)]">{{ t('workExtra.notConfigured') }}</p><div v-if="!inventoryOnly" class="flex justify-end"><UButton type="button" color="neutral" variant="soft" size="sm" :loading="pending" @click="saveInventory" >{{ t('workExtra.saveDraft') }}</UButton></div></div></details></section><template v-if="!inventoryOnly"><section v-if="selected.checklist.length"><p class="mb-2 font-semibold">{{ t('common.checklist') }}</p><div class="space-y-1"><UCheckbox v-for="item in selected.checklist" :key="item.label" v-model="item.checked" :label="item.label" class="min-h-11 items-center" /></div></section><UFormField :label="t('workExtra.comment')" class="w-full"><UTextarea v-model="comment" class="w-full" /></UFormField><UCheckbox v-model="hasProblem" :label="t('workExtra.hasProblem')" class="min-h-11 items-center font-medium" /><UFormField v-if="hasProblem" :label="t('workExtra.problemDescription')" class="w-full"><UTextarea v-model="problemDescription" class="w-full" required /></UFormField><UFormField :label="t('work.photos')" :help="t('workExtra.optional')"><UInput type="file" accept="image/*" @change="selectPhoto" /></UFormField></template><UAlert v-if="error" color="error" variant="soft" :description="error" /></form></template><template #footer><div v-if="selected" class="form-actions form-actions--footer"><UButton type="button" color="neutral" variant="ghost" @click="completeOpen = false">{{ t('work.cancel') }}</UButton><UButton type="submit" form="complete-work-form" :loading="pending">{{ inventoryOnly ? t('workExtra.saveDraft') : t('work.complete') }}</UButton></div></template></USlideover>

    <DeleteConfirmModal v-model:open="deleteOpen" :title="workToDelete?.kind === 'cleaning' ? t('workExtra.deleteCleaningQuestion') : t('workExtra.deleteTaskQuestion')" :description="t('work.deleteWorkDescription')" :loading="pending" :error="error" @confirm="removeWork" />
    <UModal v-model:open="quickCompleteOpen" :title="t('workExtra.quickCompleteTitle')"><template #body><div class="space-y-5"><p>{{ t('workExtra.quickCompleteDescription') }}</p><UAlert v-if="error" color="error" variant="soft" :description="error" /><div class="form-actions"><UButton color="neutral" variant="ghost" @click="quickCompleteOpen = false">{{ t('work.cancel') }}</UButton><UButton color="primary" :loading="pending" @click="quickComplete">{{ t('work.complete') }}</UButton></div></div></template></UModal>
  </section>
</template>

...
<template>
  <section class="page-wrap space-y-6">
    ...
    <USlideover v-model:open="detailOpen" :title="t('problems.card')"><template #body><div v-if="selected" class="space-y-6"><UAlert v-if="error" color="error" variant="soft" :description="error" /><UAlert v-if="selected.deletionRequestedAt" color="warning" variant="soft" title="Запрошено удаление"><template #actions><UButton color="error" size="sm" :loading="pending" @click="decideDeletion(true)">Одобрить</UButton><UButton color="neutral" size="sm" :loading="pending" @click="decideDeletion(false)">Отклонить</UButton></template></UAlert><div class="flex items-start justify-between gap-3"><div><p class="font-semibold">{{ selected.apartment.hotel.name }} · {{ selected.apartment.name }}</p><p class="mt-1 text-sm text-[var(--color-muted)]">{{ t('problems.createdBy', { name: selected.createdBy?.name ?? '—', date: formatDateTime(selected.createdAt) }) }}</p></div><StatusBadge :label="selected.resolvedAt ? t('problems.resolved') : t('problems.open')" :tone="selected.resolvedAt ? 'success' : 'danger'" /></div><UFormField :label="t('problems.description')"><UTextarea v-model="selected.description" class="w-full" :rows="5" :disabled="!selectedOpen" /></UFormField><div v-if="selected.cleaningId || selected.sourceTaskId" class="text-sm font-medium text-[var(--color-primary)]"><NuxtLink v-if="selected.cleaningId" :to="`/cleanings/${selected.cleaningId}`">{{ t('problems.openCleaning') }}</NuxtLink><NuxtLink v-else :to="`/tasks/${selected.sourceTaskId}`">Открыть исходную задачу</NuxtLink></div><section><div class="flex items-center justify-between gap-3"><h3 class="font-semibold">Задача решения</h3><UButton v-if="selectedOpen && !selected.activeSolutionTask" color="neutral" variant="soft" class="min-h-11 active:scale-[0.96] transition-transform" @click="openTask">Назначить решение</UButton></div><div v-if="selected.activeSolutionTask" class="mt-3 flex items-center justify-between rounded-xl bg-[var(--color-surface-muted)] p-3"><NuxtLink :to="`/tasks/${selected.activeSolutionTask.id}`" class="font-medium text-[var(--color-primary)]">{{ selected.activeSolutionTask.title }}</NuxtLink><StatusBadge :label="selected.activeSolutionTask.status" tone="info" /></div><div v-if="selected.solutionTasks?.some(task => task.status === 'completed')" class="mt-3 space-y-2"><p class="text-sm font-medium text-[var(--color-muted)]">Завершённые задачи</p><NuxtLink v-for="task in selected.solutionTasks?.filter(task => task.status === 'completed')" :key="task.id" :to="`/tasks/${task.id}`" class="block text-sm text-[var(--color-primary)]">{{ task.title }}</NuxtLink></div></section><div v-if="selected.attachments.length" class="grid grid-cols-2 gap-2"><a v-for="attachment in selected.attachments" :key="attachment.id" :href="`/api/attachments/${attachment.id}/file`" target="_blank" rel="noreferrer" class="overflow-hidden rounded-xl bg-[var(--color-surface-muted)]"><img :src="`/api/attachments/${attachment.id}/file?variant=card`" :alt="attachment.fileName" class="aspect-[2/1] w-full object-cover" /><span class="block truncate p-2 text-xs">{{ attachment.fileName }}</span></a></div><UFormField v-if="selectedOpen" :label="t('problems.photos')"><UInput type="file" accept="image/*" multiple @change="photos = Array.from(($event.target as HTMLInputElement).files ?? [])" /></UFormField><section><div class="flex items-center justify-between gap-3"><h3 class="font-semibold">{{ t('problems.expenses') }}</h3><UButton v-if="selectedOpen" color="neutral" variant="soft" icon="i-lucide-plus" class="min-h-11" @click="openExpense()">{{ t('problems.addExpense') }}</UButton></div><p class="mt-3 text-right text-sm font-semibold">{{ t('problems.total') }}: {{ formatEuro(selected.totalExpenseEur) }}</p></section></div></template><template #footer><div v-if="selected" class="form-actions form-actions--footer"><template v-if="selectedOpen"><UButton color="error" variant="ghost" @click="deleteOpen = true">{{ t('common.delete') }}</UButton><UButton color="neutral" variant="outline" @click="resolveOpen = true">{{ t('problems.resolve') }}</UButton><UButton :loading="pending" @click="saveDescription">{{ t('common.save') }}</UButton></template><template v-else><UButton color="neutral" variant="outline" :loading="pending" @click="reopen">{{ t('problems.reopen') }}</UButton></template></div></template></USlideover>

...
const form = reactive({ apartmentId: '', description: '' })
...
watch(problems, async rows => {
  ...
  if (!problemId || !rows.some(problem => problem.id === problemId)) return
  await openDetail(rows.find(problem => problem.id === problemId)!)
  await navigateTo({ path: '/problems', query: { ...route.query, problemId: undefined } }, { replace: true })
}, { immediate: true })

function openCreate() {
  Object.assign(form, { apartmentId: apartments.value[0]?.id ?? '', description: '' })
  photos.value = []; error.value = ''; validation.reset(); formOpen.value = true
}
...
async function saveProblem() {
  ...
  try {
    ...
    formOpen.value = false; photos.value = []; await refresh(); await openDetail(created)
...
async function saveDescription() {
  ...
  pending.value = true; error.value = ''
  try { selected.value = await $fetch<Problem>(`/api/problems/${selected.value.id}`, { method: 'PATCH', body: { description: selected.value.description } }); await uploadPhotos(selected.value.id); photos.value = []; await refresh() }
  catch (cause: any) { error.value = cause?.data?.statusMessage ?? t('common.error') }
  finally { pending.value = false }
}
async function resolve() {
  if (!selected.value) return
  pending.value = true; error.value = ''
  try { selected.value = await $fetch<Problem>(`/api/problems/${selected.value.id}/resolve`, { method: 'POST', body: { resolutionComment: resolutionComment.value } }); resolveOpen.value = false; await refresh() }
  catch (cause: any) { error.value = cause?.data?.statusMessage ?? t('common.error') }
  finally { pending.value = false }
}
...
async function removeProblem() {
  if (!selected.value) return
  pending.value = true; error.value = ''
  try { await $fetch(`/api/problems/${selected.value.id}`, { method: 'DELETE' }); deleteOpen.value = false; detailOpen.value = false; selected.value = null; await refresh() }
  catch (cause: any) { error.value = cause?.data?.statusMessage ?? t('common.error') }
  finally { pending.value = false }
}
</script>
...
<template>
  <section class="page-wrap space-y-6">
    ...
    <USlideover v-model:open="formOpen" :title="t('problems.new')"><template #body><UForm :key="validation.formKey.value" id="problem-form" :state="form" :validate="validate" :validate-on="validation.validateOn.value" novalidate class="form-grid" @error="validation.onError" @submit="saveProblem"><UFormField name="apartmentId" :label="t('expenses.apartment')"><USelect v-model="form.apartmentId" :items="apartmentOptions.slice(1)" class="w-full" /></UFormField><UFormField name="description" :label="t('problems.description')" class="w-full"><UTextarea v-model="form.description" class="w-full" :rows="5" /></UFormField><UFormField :label="t('problems.photos')" class="w-full"><UInput type="file" accept="image/*" multiple @change="photos = Array.from(($event.target as HTMLInputElement).files ?? [])" /></UFormField><UAlert v-if="error" color="error" variant="soft" :description="error" /></UForm></template><template #footer><div class="form-actions form-actions--footer"><UButton color="neutral" variant="ghost" @click="formOpen = false">{{ t('common.cancel') }}</UButton><UButton type="submit" form="problem-form" :loading="pending">{{ t('common.save') }}</UButton></div></template></USlideover>

    <USlideover v-model:open="detailOpen" :title="t('problems.card')"><template #body><div v-if="selected" class="space-y-6"><UAlert v-if="error" color="error" variant="soft" :description="error" /><div class="flex items-start justify-between gap-3"><div><p class="font-semibold">{{ selected.apartment.hotel.name }} · {{ selected.apartment.name }}</p><p class="mt-1 text-sm text-[var(--color-muted)]">{{ t('problems.createdBy', { name: selected.createdBy?.name ?? '—', date: formatDateTime(selected.createdAt) }) }}</p></div><StatusBadge :label="selected.resolvedAt ? t('problems.resolved') : t('problems.open')" :tone="selected.resolvedAt ? 'success' : 'danger'" /></div><UFormField :label="t('problems.description')"><UTextarea v-model="selected.description" class="w-full" :rows="5" :disabled="!selectedOpen" /></UFormField><div v-if="selected.cleaningId"><NuxtLink :to="`/cleanings/${selected.cleaningId}`" class="text-sm font-medium text-[var(--color-primary)]">{{ t('problems.openCleaning') }}</NuxtLink></div><div v-if="selected.attachments.length" class="grid grid-cols-2 gap-2"><a v-for="attachment in selected.attachments" :key="attachment.id" :href="`/api/attachments/${attachment.id}/file`" target="_blank" rel="noreferrer" class="overflow-hidden rounded-xl bg-[var(--color-surface-muted)]"><img :src="`/api/attachments/${attachment.id}/file?variant=card`" :alt="attachment.fileName" class="aspect-[2/1] w-full object-cover" /><span class="block truncate p-2 text-xs">{{ attachment.fileName }}</span></a></div><UFormField v-if="selectedOpen" :label="t('problems.photos')"><UInput type="file" accept="image/*" multiple @change="photos = Array.from(($event.target as HTMLInputElement).files ?? [])" /></UFormField><template v-if="selected.resolvedAt"><div class="rounded-xl bg-[var(--color-surface-muted)] p-4"><p class="font-medium">{{ t('problems.resolvedBy', { name: selected.resolvedBy?.name ?? '—', date: formatDateTime(selected.resolvedAt) }) }}</p><p v-if="selected.resolutionComment" class="mt-2 whitespace-pre-line text-sm text-[var(--color-muted)]">{{ selected.resolutionComment }}</p></div></template><section><div class="flex items-center justify-between gap-3"><h3 class="font-semibold">{{ t('problems.expenses') }}</h3><UButton v-if="selectedOpen" color="neutral" variant="soft" icon="i-lucide-plus" class="min-h-11" @click="openExpense()">{{ t('problems.addExpense') }}</UButton></div><div v-if="selected.expenses.length" class="mt-3 divide-y divide-[var(--color-line)]"><div v-for="expense in selected.expenses" :key="expense.id" class="flex min-h-14 items-center gap-3 py-2"><button type="button" class="min-w-0 flex-1 text-left" :disabled="!selectedOpen" @click="openExpense(expense)"><p class="truncate font-medium">{{ expense.description }}</p><p class="text-sm text-[var(--color-muted)]">{{ formatDate(expense.occurredOn) }}</p></button><strong class="tabular-nums">{{ formatEuro(expense.amountEur) }}</strong><UButton v-if="selectedOpen" color="error" variant="ghost" icon="i-lucide-trash-2" class="min-h-11 min-w-11" :aria-label="t('common.delete')" @click="removeExpense(expense)" /></div></div><p class="mt-3 text-right text-sm font-semibold">{{ t('problems.total') }}: {{ formatEuro(selected.totalExpenseEur) }}</p></section></div></template><template #footer><div v-if="selected" class="form-actions form-actions--footer"><template v-if="selectedOpen"><UButton color="error" variant="ghost" @click="deleteOpen = true">{{ t('common.delete') }}</UButton><UButton color="neutral" variant="outline" @click="resolveOpen = true">{{ t('problems.resolve') }}</UButton><UButton :loading="pending" @click="saveDescription">{{ t('common.save') }}</UButton></template><template v-else><UButton color="neutral" variant="outline" :loading="pending" @click="reopen">{{ t('problems.reopen') }}</UButton></template></div></template></USlideover>

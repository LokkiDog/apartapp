...
describe('stay history UI', () => {
  it('renders history and uncleaned panels in both list modes', () => {
    ...
    expect(row.match(/:cash-amount-eur="stay\.cashAmountEur" :cash-task-state="stay\.cashTaskState"/g)).toHaveLength(2)
    const icons = readFileSync('src/pages/calendar/StayServiceIcons.vue', 'utf8')
    expect(icons).toContain("typeof props.cashAmountEur === 'number' && props.cashAmountEur > 0")
    expect(icons).toContain('name="i-lucide-euro"')
    expect(icons).toContain('stay-service-icon--cash-${cashTaskState}')
    const styles = readFileSync('src/app/styles/main.css', 'utf8')
    expect(styles).toContain('.stay-service-icon--cash-none')
    expect(styles).toContain('.stay-service-icon--cash-pending')
    expect(styles).toContain('.stay-service-icon--cash-collected')
    const service = readFileSync('server/modules/stay/stay.service.ts', 'utf8')
    expect(service).toContain("new Map<string, 'pending' | 'collected'>()")
    expect(service).toContain("if (cashTask.collectedAt) cashTaskStateByStayId.set(cashTask.stayId, 'collected')")
    expect(service).toContain("const cashTaskState = cashTaskStateByStayId.get(stay.id) ?? 'none'")
  })
})
